...
class Angles():
    ...
    def ObSu_Calc(object_x,object_y,subject_x,subject_y):
        ...
        return (math.atan(ver_dist/hor_dist)*57.296 )*-1
...
##            self.True_angle =360-( Angles.True_angle(self.rot,self.flipped))##Angles.ObSu_Calc(self.rect.x+self.image.get_width()//2,self.rect.y+self.image.get_height()//2,mouse_x,mouse_y)
...
##ATTENTION:
##HITSCAN WAS ALMOST DESIGNED FOR A CERTAIN PROJECT; IT IS INEFFICIENT AND SHOULD NOT BE REUSED!
...
class Animation():
    ...
    def ani_cycle(self, sprite_list,frame_delay):
        self.frametracker+=1
        if self.frametracker>FPS:
            self.frametracker = 0
        try:
            if self.frametracker>frame_delay:
                self.orig = sprite_list[self.anitrack]
                self.anitrack+=1
                self.frametracker = -1                   
        except IndexError:
            self.anitrack = 1
